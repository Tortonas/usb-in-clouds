<?php
	...
	function FindHowManySpaceFilesTake()
	{
		$directory = "files";
		$size = 0;
		//Algoritmas suskaičiuoti kiek vietos užima files direktorija
		...
		//Algoritmas kuris randa kiek hidden files užema vietos.
		foreach (glob(rtrim($directory, '/').'/*', GLOB_NOSORT) as $each) 
		{
			$size += is_file($each) ? filesize($each) : folderSize($each);
		}
	...
	function CheckSessionLoggings()
	{
		...
		if (mysqli_num_rows($sqlResultsCheckAdmins) > 0) 
		{
			...
			{	
				if($row['nick'] == $_SESSION['nick'] && $row['password'] == $_SESSION['password'])
				{
